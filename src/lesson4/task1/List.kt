...
fun times(a: List<Double>, b: List<Double>): Double {
    var c: Double = 0.0
    if (a.isEmpty() == true && b.isEmpty() == true) 0.0
    else {
...
fun accumulate(list: MutableList<Double>): MutableList<Double> {
    ...
    var previous: Double = list[0]
    for (i in 1 until list.size) {
        list[i] = previous + list[i]
        previous = list[i]
    }
    return list
}
...
fun revert(list: MutableList<Int>): List<Int> {
    val revertList = mutableListOf<Int>()
    var element: Int
    for (i in 0..list.size - 1) {
        element = list[list.size - i - 1]
        revertList.add(element)
    }
    return revertList.toList()
}
...
fun convertToString(n: Int, base: Int): String {
    ...
    for (i in 0..digits.size - 1) {
        string = string + replaceToChar(digits[i])
    }
    return string
}

fun replaceToChar(input: Int): Char =
        if (input < 10) ('0'.toInt() + input).toChar()
        else ('a'.toInt() + (input - 10)).toChar()
...
fun decimal(digits: List<Int>, base: Int): Int {
    ...
    for (i in 0..n - 1) {
        result = result + digits[i] * base.toDouble().pow(n - 1 - i).toInt()
    }
    return result
}
...
fun decimalFromString(str: String, base: Int): Int {
    val digits = str.toList().map { replaceToDigits(it) }
    return decimal(digits, base)
}


fun replaceToDigits(input: Char): Int {
    val k: Int = input.toInt() - '0'.toInt()
    return if (k in 0..9) k
    else input.toInt() - 'a'.toInt() + 10
}
